...
export function IdentityCard({ membership }: Props) {
  ...
  return (
    <div className="flex flex-col items-center rounded-card border border-line bg-surface p-[18px] text-center shadow-[0_8px_22px_rgba(20,40,30,0.08)]">
      ...
      <div className="relative -mt-[54px]">
        ...
        />
        {/* Bespoke identity avatar — gradient tile with an emoji fallback and an
            attached photo uploader, so it stays custom rather than using <Avatar>. */}
        ...
        <button
          ...
          className="absolute bottom-0 right-0 flex h-[30px] w-[30px] items-center justify-center rounded-full border-[3px] border-surface bg-amber text-amber-ink"
        >
          <svg
            width="14"
            height="14"
      ...
      <div className="mt-3 text-display font-bold tracking-tight text-content">{fullName}</div>

      <div className="mt-2 flex gap-[7px]">
        <Badge tone="pine">
          {tribe.emoji} {tribe.name}
        </Badge>
        <Badge tone="muted">{t.profile.roles[role]}</Badge>
      </div>
...
function Stat({
  ...
  return (
    <div className={`flex-1 ${bordered ? 'border-x border-line' : ''}`}>
      <div className={`text-subhead font-extrabold ${accent ? 'text-amber' : 'text-content'}`}>
        {value}
      </div>
