...
import { orgLandingPath, usesOrgSurface } from '@/utils/roles'
...
export function Onboarding() {
  ...
  // A signed-in organizer/manager never uses the participant landing — send them to
  // their own surface (onboarding if unfinished, dashboard otherwise).
  if (usesOrgSurface(authUser?.role) && authUser) {
    return <Navigate to={orgLandingPath(authUser)} replace />
  }
  ...
  const handleLogin = (phone: string) => {
    ...
    login.mutate(
      { phone: digits },
      {
        onSuccess: (user) => {
          setPhone(digits)
          // An organizer/manager who signs in by phone here belongs on their own
          // surface, not the participant dashboard (email is their real entry, but a
          // phone login still resolves their account server-side).
          if (usesOrgSurface(user.role)) {
            navigate(orgLandingPath(user), { replace: true })
